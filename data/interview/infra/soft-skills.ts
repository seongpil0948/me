...
export const infraSoftSkillsQuestions: LocalizedInterviewQuestion[] = [
  {
    ...
    answer: {
      ko:
        "장애 대응하면서 로그 18시간 뒤지던 경험이 제일 힘들었습니다. '이건 엔지니어링이 아니라 운에 맡기는 거구나'라고 느꼈죠. 그때부터 Observability를 최우선으로 두게 되었습니다. 측정할 수 없으면 개선할 수 없습니다.\n\n" +
        "**Observability First**: 모든 시스템 변경 전에 먼저 '이 상태를 어떻게 관찰할 것인가'를 정의합니다. OpenTelemetry 기반으로 Trace-Metric-Log를 하나의 Trace ID로 연결하는 구조를 갖추면, 장애가 발생해도 Grafana에서 Root Cause를 10분 내 찾을 수 있습니다. MTTD 18시간 → 10분 단축이 이 철학의 결과입니다.\n\n" +
        "**Infrastructure as Code**: 수동 설정은 결국 누군가 실수하게 되어 있습니다. Terraform + Jenkins 파이프라인에 Destroy Guard를 추가한 것도 이 철학에서 나왔습니다. '무엇을 했나'가 아니라 '왜 그렇게 했나'가 git commit에 남아야 합니다.\n\n" +
        "**시스템 방어 설계**: 처음엔 '누가 실수했나'를 찾았는데, 지금은 '왜 시스템이 이런 실수를 허용했나'를 먼저 봅니다. 사람은 실수하게 되어 있으니 시스템이 방어해야 하죠.\n\n" +
        "**운영 복잡도 우선 평가**: 신기술 도입 시 '이 기술이 장애 나면 새벽 2시에 내가 혼자 복구할 수 있나?'를 먼저 물어봅니다. Rook-Ceph는 강력하지만 운영 복잡도가 높아서 모니터링 체계와 runbook을 먼저 만들고 도입했습니다.",
  ...
  },
  {
    ...
    category2: "Philosophy",
    question: {
      ko: "SRE와 DevOps의 차이를 어떻게 이해하습니까? 현업에서 두 역할을 어떻게 구분해서 일하습니까?",
    },
    answer: {
      ko:
        ...
        "핵심 차이는 **Error Budget**입니다. SRE는 '이만큼의 장애는 허용한다'는 에러 버짓을 정하고, 그 범위 내에서 빠른 배포와 안정성의 균형을 잡아요. DevOps는 이런 명시적 숫자 없이 '더 빠르게, 더 안전하게'를 추구하는 경향이 있습니다.\n\n" +
        "**현업에서 실제로 하는 일**\n\n" +
        ...
        "**실제로 가장 중요했던 판단**\n\n" +
        "결제 API P99 latency가 8초일 때 '당장 스케일업으로 해결하자' vs '근본 원인(N+1 쿼리)을 해결하자' 사이에서 SRE 관점을 취했습니다. 임시 조치로 Connection Pool 크기를 늘리고(DevOps), 근본 원인은 백엔드 팀과 협업해서 쿼리 최적화로 해결했습니다(SRE). 두 관점의 균형이 중요합니다.",
      en: "A concise Philosophy answer covering real production context, key decision trade-offs, measurable outcomes, and takeaways.",
    },
  },
  {
    ...
    category2: "Philosophy",
    question: {
      ko: "온프레미스와 클라우드 중 무엇을 선택할지 어떤 기준으로 판단하습니까?",
    },
    answer: {
      ko:
        "이 판단을 가장 많이 해본 게 TheShop IDC 이전 프로젝트였습니다. 결론부터 말하면 **'워크로드 성격에 따라 최적의 환경이 다르다'**입니다. 저는 지금 EKS + 온프레미스 K8s + ECS를 동시에 운영하며 이 철학을 실천하고 있습니다.\n\n" +
        "**온프레미스를 유지한 워크로드**\n\n" +
        ...
        "- **Rook-Ceph 분산 스토리지**: EKS EBS는 단일 AZ 제약이 있습니다. RWX (Read-Write-Many)가 필요한 워크로드는 CephFS를 통해 온프레미스에서 처리.\n\n" +
        "**클라우드(EKS)를 선택한 워크로드**\n\n" +
