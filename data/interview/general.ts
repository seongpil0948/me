...
export const generalQuestions: LocalizedInterviewQuestion[] = [
  ...
  // Problem Solving
  {
    ...
    question: {
      ko: "가장 어려웠던 기술적 문제는 무엇이고 어떻게 해결했나요?",
  ...
  // Additional General Questions
  {
    ...
    answer: {
      ko:
        "엔지니어로서 가장 큰 강점을 꼽으라면 '보이지 않는 것을 보이게 만드는 능력'이라고 생각해요.\n\n" +
        "제가 TheShop에 합류했을 때 장애가 나면 15개 서버에 tail -f를 펼쳐놓고 18시간씩 로그를 뒤지는 방식으로 운영했어요. 처음 이 상황을 봤을 때 '이건 엔지니어링이 아니라 운에 맡기는 거다'라고 느꼈죠. 문제를 찾고 싶은데 도구가 없으면 문제 자체가 안 보이잖아요.\n\n" +
        "그래서 먼저 1개 서비스에 OpenTelemetry를 조용히 붙여봤어요. 별도 승인이나 프로젝트 등록 없이 POC를 만들고, 일주일 후에 MTTD가 18시간에서 10분으로 줄었다는 데이터를 들고 경영진을 찾아갔습니다. '이렇게 하면 좋겠습니다'가 아니라 '이미 검증했고, 증거가 있습니다'라고 말했죠. 즉시 승인이 났고 전사 확대로 이어졌어요.\n\n" +
        "이게 저의 패턴이에요. 말보다 작동하는 결과물을 먼저 만들고, 데이터로 설득합니다. Redis 캐시 히트율 개선도, Kafka 파티셔닝 최적화도, 모두 POC → 수치 → 설득 순서였어요. 추상적인 논의보다 구체적인 산출물이 의사결정을 훨씬 빠르게 만들거든요.\n\n" +
        ...
        "지금 다시 한다면 팀원 온보딩을 더 일찍 챙겼을 것 같아요. 혼자 빠르게 구축하다 보니 중요한 시스템이 제가 없으면 아무도 못 건드리는 상황이 생겼거든요. 인프라에만 Single Point of Failure가 생기는 게 아니더라고요.",
      en:
  ...
  },
  {
    ...
    answer: {
      ko:
        "제 약점은 빠르게 실행하려다가 팀원들을 앞서나가는 경향입니다. 지금도 의식적으로 관리하고 있어요.\n\n" +
        "OpenTelemetry 도입 초기에 이런 실수를 했어요. POC를 완성하고 바로 전사 도입을 추진했는데, 일부 팀원들이 '갑자기 왜 이걸 배워야 하냐'며 반발했죠. 제 머릿속엔 이미 전체 그림이 그려져 있었지만, 다른 사람들은 아직 필요성조차 못 느끼고 있었어요. 결과가 아무리 좋아도 과정에서 사람들이 소외되면 지속되지 않더라고요.\n\n" +
        ...
        "여전히 '이거 지금 바로 하면 3일 안에 끝날 텐데'라는 조급함이 올라올 때가 있어요. 그럴 때 '혼자 빠르게 가는 것보다 팀과 함께 멀리 가는 게 더 중요하다'는 것을 스스로에게 상기시킵니다.\n\n" +
        "지금 다시 한다면 도입 전에 Why를 먼저 공유했을 거예요. What(뭘 할 것인지)을 먼저 보여주기 전에 Why(왜 지금 이게 필요한지)부터 팀이 이해하게 만들었으면 훨씬 수월했을 겁니다.",
      en:
  ...
  },
  {
    ...
    answer: {
      ko:
        "LG IXI Studio 프로젝트에 합류했을 때 SSE(Server-Sent Events)를 2주 안에 프로덕션 수준으로 구현해야 했는데, 그때까지 SSE를 써본 적이 없었어요.\n\n" +
        "처음엔 막막했지만 제 학습 순서는 항상 비슷해요. 공식 문서를 먼저 파고들고, 가장 단순한 프로토타입을 직접 만들어보고, 실제 코드에서 패턴을 찾는 거예요. MDN의 SSE 스펙 문서를 읽고, Node.js 백엔드와 Vue.js 클라이언트에서 각각 가장 작은 구현부터 만들었습니다. 연결 유지, 재연결 로직, 에러 처리를 하나씩 실험하면서 엣지 케이스를 파악했어요.\n\n" +
        "가장 까다로웠던 건 AI 모델의 스트리밍 응답을 끊김 없이 프론트엔드까지 전달하는 부분이었어요. 응답이 글자 단위로 오는 특성상 버퍼링을 어떻게 처리하느냐가 UX에 직접 영향을 줬거든요. 최종적으로 2주 만에 프로덕션 배포를 완료했고, 실시간 AI 스트리밍이 안정적으로 동작했습니다.\n\n" +
        ...
        "지금 다시 한다면 처음부터 재연결 로직을 더 견고하게 설계했을 거예요. 프로덕션에서 네트워크 불안정이 생겼을 때 클라이언트 재연결이 일부 케이스에서 실패하는 걸 나중에 발견했거든요.",
      en:
  ...
  },
  {
    ...
    category2: "Problem Solving",
    question: {
      ko: "복잡한 프로덕션 이슈를 디버깅할 때 어떻게 접근하나요?",
      en: "How do you approach debugging complex production issues?",
    },
    answer: {
      ko:
        "프로덕션 디버깅에서 가장 중요한 건 침착하게 증거를 따라가는 것이에요.\n\n" +
        "TheShop에서 기억에 남는 케이스는 결제 API에서 간헐적으로 500 에러가 발생한 건데, 특정 시간대에만 재현되고 재현율도 5% 수준이라 잡기가 까다로웠어요. 먼저 '최근에 뭐가 바뀌었나'부터 시작했는데, 마침 그 전날 신규 기능 배포가 있었거든요.\n\n" +
        "OpenTelemetry Trace에서 결제 서비스의 응답 시간이 특정 요청에서만 3-4초로 튀는 패턴을 발견했어요. Trace를 따라가니 DB 쿼리에서 커넥션을 기다리는 구간이 길었고, 거기서 Connection Pool 고갈 가설을 세웠습니다. hikaricp.connections.pending 메트릭을 보니 특정 시간대에 5 이상으로 치솟는 게 확인됐어요.\n\n" +
        "신규 기능 코드를 보니 특정 조건에서 DB 쿼리를 N+1로 호출하는 로직이 있었어요. 즉시 임시 조치로 Connection Pool 크기를 늘려서 증상을 완화하고, 근본 원인인 쿼리를 최적화했습니다. 이후 CI/CD에 쿼리 성능 테스트를 추가해서 같은 문제가 재발하지 않도록 했어요.\n\n" +
        "저는 항상 '배포 이력 확인 → Trace로 느린 구간 특정 → 메트릭으로 가설 검증 → 코드 공범 찾기' 순서를 따라요. 추측으로 시작하면 시간이 배로 걸리더라고요.\n\n" +
        "지금 다시 한다면 Trace 기반 디버깅을 첫 날부터 팀 표준으로 만들었을 거예요. 처음엔 로그만 보던 팀원들이 Trace 분석 방법을 배운 후에 평균 디버깅 시간이 확연히 줄었거든요.",
  ...
  },
  {
    ...
    question: {
      ko: "비기술 이해관계자에게 기술 개념을 어떻게 설명하나요?",
  ...
  },
  {
    ...
    question: {
      ko: "딥러닝과 AI에 관심을 갖게 된 계기는 무엇이고, 그것이 현재 AI 에이전트 개발로 어떻게 이어졌나요?",
    ...
    answer: {
      ko:
        ...
        "첫 직장인 AI 추천 솔루션 회사에서 ML 연구원들과 밀접하게 협업했습니다. 제 역할은 '풀스택 개발자'였지만, AI 연구원들이 개발한 모델을 실제 서비스로 연결하는 작업을 담당했어요.\n\n" +
        "구체적으로는:\n" +
        "- **추천 엔진 서빙**: n2 알고리즘(ANN) 기반 상품 추천 모델을 FastAPI로 서빙. 사용자 벡터와 상품 벡터의 유사도 계산이 핵심이었는데, 이때 임베딩 개념을 처음 깊이 이해했어요.\n" +
        "- **NLP 데이터 파이프라인**: 패션 매거진 40개 쇼핑몰 데이터를 수집해 TF-IDF 기반 키워드 추출과 트렌드 분석 파이프라인을 구축했어요. 텍스트를 벡터로 변환하고 유사도를 계산하는 과정이 흥미로웠습니다.\n" +
        "- **ML 파이프라인**: Argo Workflows + RabbitMQ로 데이터 수집 → 전처리 → 모델 학습 → 배포 전 과정을 자동화했어요.\n\n" +
        "이때 배운 핵심은 **'AI 모델은 아무리 좋아도 서빙 인프라가 없으면 가치를 못 낸다'**는 것이었습니다. ML 연구원이 만든 모델을 운영 환경에 배포하고, 모니터링하고, 스케일링하는 과정에서 인프라가 얼마나 중요한지 깨달았죠.\n\n" +
        "**가속: Abacus에서 LG IXI AI 플랫폼 (2023-2024)**\n\n" +
        "LG에서 생성형 AI SaaS 플랫폼(IXI Studio) 프론트엔드 개발을 담당했어요. SSE로 AI 응답을 실시간 스트리밍하고, AI가 생성한 코드를 CodeMirror 에디터로 편집하는 기능을 구현했습니다.\n\n" +
        "여기서 **'LLM이 어떻게 동작하는지'**를 사용자 관점에서 깊이 이해했어요. Prompt Engineering이 결과에 미치는 영향, Streaming 응답의 UX, 사용자 인터페이스와 AI 연결 방식을 직접 경험했죠.\n\n" +
        ...
        "- **서빙 인프라**: AI 모델 서빙 경험 덕분에 Bedrock Converse API의 Prompt Caching, 모델 티어 분리(Claude Sonnet/Haiku/Opus)를 비용 효율적으로 설계할 수 있었죠.\n" +
        "- **A2A/MCP 아키텍처**: ML 마이크로서비스 연동 경험이 에이전트 간 통신 프로토콜 설계에 응용됐어요.\n\n" +
        "**결론**\n\n" +
  ...
  },
  {
    ...
    answer: {
      ko:
        ...
        "TheShop EKS + 온프레미스 클러스터 운영을 혼자 담당하면서, 팀 Jenkins 표준과 AWS 콘솔 수동 작업이 뒤섞이는 문제가 있었어요. Security Group을 콘솔에서 만들었다가 나중에 '누가 이거 추가했지?', 'ECR 레포 목록이 어떻게 되지?' 하는 상황이 반복됐습니다. 그래서 shop-iac 프로젝트로 AWS 인프라를 Terraform IaC로 전환했어요.\n\n" +
        ...
        "각 환경은 독립된 S3 버킷과 DynamoDB 락 테이블을 가집니다. 절대 상태 파일이 공유되지 않아요. DEV 실수가 PRD에 영향을 주는 상황을 원천 차단했죠.\n\n" +
        "**Jenkins CI/CD 파이프라인**\n\n" +
        ...
        "**핵심 교훈**\n\n" +
        "IaC는 인프라를 '코드로 표현하는 것'이 아니라 '감사 가능하고 재현 가능한 상태로 관리하는 것'이에요. Destroy Guard 하나가 실수로 ECR 레포를 날리는 사고를 막아줄 수 있죠. 인프라 규모가 작아도 처음부터 IaC로 가는 게 맞다고 생각합니다.",
  ...
  },
  {
    ...
    question: {
      ko: "소프트웨어 엔지니어링에 대한 AI의 영향에 대해 어떻게 생각하나요?",
