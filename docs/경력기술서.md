...
| 2023.05 ~ 2024.06 | 애버커스             | 선임연구원 (Frontend Lead) | Kubernetes, RabbitMQ, Three.js             |
...
- **기간**: 2024년 06월 ~ 현재 (재직중)
- **직급**: 선임연구원
- **소속**: TheShop 이커머스 플랫펼 지원팀
- **회사 소개**: 대웅 그룹 IT 계열사
- **담당 업무**: 
  - Legacy 시스템 (Oracle/Tomcat) → Next.js/Spring 이관 프로젝트 코어 모듈, 빠대, 애플리케이션 설계
  - TheShop 플랫펼 인프라 최적화, 분산 시스템 관리, 신규 서비스 아키텍처 설계 주도
...
#### 1.1. Legacy 시스템 현대화 (Oracle/Tomcat → Next.js/Spring)

**Challenge**

- 10년 이상 운영된 Oracle DB + Tomcat 기반 모놀리식 시스템
- 유지보수 어려움 및 신규 기능 개발 속도 저하
- 레거시 시스템과 신규 MSA의 공존 필요
- 데이터 이관 및 동기화 복잡성
...
- **Next.js 프론트엔드** 및 **Spring Boot 백엔드** 아키텍처 설계
- 코어 모듈 설계: 인증/인가, API 통신, 상태 관리 등 재사용 가능한 핵심 모듈 구축
- 프로젝트 뼈대(Scaffold) 구성: 프로젝트 구조, 코딩 컨벤션, CI/CD 파이프라인 구축
- 레거시와 신규 시스템 간 점진적 마이그레이션 전략 수립

_Airflow 기반 데이터 이관 파이프라인_

- **Apache Airflow 5-server 클러스터** 구축 및 운영
- **통계성 데이터 배치 처리**: 일/주/월 단위 집계 데이터 자동 생성 및 이관
- **Data Lake 구축**: AWS Glue + Athena를 통한 장기 데이터 저장소 구축 (10년 보관)
- **준실시간 CDC (Change Data Capture)**: Oracle → 신규 DB 데이터 동기화
- **200개 이상 DAG** 관리: 복잡한 데이터 의존성 및 작업 스케줄링

**Achievement**

- 신규 시스템 코어 아키텍처 설계 및 개발 환경 구축 완료
- Airflow 클러스터로 데이터 이관 자동화, 수동 작업 대비 90% 시간 단축
- Data Lake 구축으로 로그 조회 기간 142배 확장 (7일 → 10년)
- CDC를 통한 준실시간 데이터 동기화로 레거시-신규 시스템 공존 환경 구현
- 200개 이상 배치 작업 안정적 운영 및 모니터링 체계 확립

**사용 기술**

- Next.js, Spring Boot, Oracle DB
- Apache Airflow 5-server cluster
- AWS Glue, Athena, S3
- Python (Airflow DAG, Data Pipeline)
- CDC tools for Oracle

---

#### 1.2. 모니터링 시스템 고도화 (Legacy End-to-End 관측 환경 구축)
...
_Apache Airflow 오케스트레이션 (데이터 이관 핵심 역할)_

- **5대 서버 클러스터** 구성 및 운영 총괄
- **통계성 데이터 처리**: 일/주/월 단위 집계 데이터 자동 생성 및 이관 (50+ DAGs)
- **Data Lake 파이프라인**: Oracle → S3 → Glue → Athena 데이터 레이크 구축 (30+ DAGs)
- **준실시간 CDC**: Legacy Oracle DB → 신규 시스템 데이터 동기화 (20+ DAGs)
- **일반 배치 작업**: 리포트 생성, 알림 발송, 데이터 정합성 체크 등 (100+ DAGs)
- 200개 이상 DAG(Directed Acyclic Graph) 관리 및 복잡한 의존성 처리
- 작업 실패 시 자동 재시도 로직 및 Slack 알림 연동

**Achievement**

- **일일 2~5천만 건** 메시지 안정적 처리
...
_CI/CD 파이프라인 및 IaC 구현_

- **GitHub 기반 CI/CD 자동화**
  - AWS CodeCommit + CloudFormation을 통한 GitHub 기반 배포 자동화
  - Infrastructure as Code(IaC)로 일관된 인프라 프로비저닝
  - 버전 관리를 통한 인프라 변경 이력 추적
  - 배포 시간 90% 단축 (2시간 → 12분)
- **Onpremise Jenkins CI/CD 경험** (이전 프로젝트)
  - GitLab(Web hook) 기반 Jenkins 파이프라인 구성
  - 빌드, 테스트, 배포 자동화
- **ArgoCD GitOps 경험** (이전 프로젝트)
  - GitHub 기반 GitOps CD 파이프라인 구현
...
- LG의 높은 보안 요구사항 충족
- 바이올렛 플랫폼이 초기 단계로 개발환경 구축에 어려움
- Gateway(Apache), Container 환경(network, resource) 수동 관리 필요
- 환경변수 수동 주입, 컨테이너 실행 리소스(프로세스 권한, 파일 관리) 상세 관리 필요
...
- Horizontal Pod Autoscaler, Notebook resource 관리
- Argo Workflows 연동
- kubectl 명령어로 low level 리소스 직접 관리
...
- Istio service mesh 환경에서 네트워킹 이슈 해결
- CVT 테스트를 통한 쿠버네티스, Istio 환경 검증
- WAF를 사용한 보안 강화
...
- LG 개발팀과 티키타카하며 바이올렛 플랫폼의 안정화에 기여
...
- 쿠버네티스, Istio 환경에서 실제 리소스를 직접 관리하는 경험의 가치
- 취득한 쿠버네티스 자격증을 기반으로 인프라 담당자와 원활한 소통
...
- 기존 SSG 프레임워크 빌드 시 CodeMirror의 브라우저 참조 이슈
- 외부 서비스로 기획 초기 디자인 수정사항이 많아 개발일정 빠듯
- 기업 특성에 맞춤 내부에서 사용할 수 있는 AI 모델 생성/관리 플랫폼 구축

**Solution**

_프로젝트 리더십_

- Frontend Project Leader로서 프로젝트 이해도 향상
- 병목점이 높은 기능(Chat, Code Editor, SSE) POC 품질 향상에 시간 투자
- 개발 일정 맞추기 위한 전략적 우선순위 설정
...
- 연결 끊김 시 자동 재연결 메커니즘
- MQTT/AMQP, Websocket, REST API, GRPC에 이어 새로운 프로토콜 SSE 경험

_Code Editor 개선_

- CodeMirror 라이브러리 프로젝트에 clone 후 Vue 코드에 맞게 오픈소스 수정
- Vite prerender build time 이슈 해결
- Syntax highlighting 최적化
- AI 코드 제안 기능 통합

_UI Tools_

- konva.js, element-ui, apache echarts 활용한 데이터 시각화
...
- Java Spring Boot
- Aura RDS
- konva.js, element-ui, apache echarts

---

#### 2.5. In-house Next.js Framework 개발

**Challenge**

- 클라이언트 프로젝트마다 반복되는 설정 및 컴포넌트 개발
- Vue.js가 주력이었던 자사에 Next.js 개발을 요구하는 프로젝트 발생
- 개발 속도 향상 필요
- 일관된 UI/UX 제공
- 각 프로젝트 테마를 빠르게 적용할 수 있는 configuration 시스템 필요
...
- **Storybook**으로 컴포넌트 문서화 및 Frontend workshop 구축
- SSR, SSG, SPA 렌더링 지원
- CORS 환경, proxy 설정이 가능한 가이드라인 제공

_Style Configuration_

- tailwind-variants를 통해 css, class as config 적용
- CSS-in-JS 없이 빠르게 스타일 적용 가능
- 모든 공통 컴포넌트에 일관된 디자인 적용

_팀 협업_

- 퍼블리싱 팀과 협업하여 품질 높은 공용 컴포넌트 제작
- Frontend Lead로서 빠른 생산성을 위한 전략 수립

**Achievement**

- 클라이언트 프로젝트 개발 속도 대폭 향상
- 재사용 가능한 고품질 컴포넌트 라이브러리 구축
- Storybook으로 개발자 간 협업 효율 증대
- 13버전 이후 대폭 변경된 AppRouter 기반 프로젝트 경험
- tailwind-variants를 통한 효율적인 스타일 관리 시스템 구축

**사용 기술**

- Next.js (AppRouter), React
...
#### 3.2. 캠핑 SNS 모바일 앱 (Campi)

**Challenge**

- 캠핑장 예약, 리뷰 및 사진 공유, 정보 제공 통합 플랫폼 부재
- 이미지 편집 기능 필요 (Pinch-to-zoom, 크롭, 회전, 필터)
- 하이브리드 앱 개발 도중 이미지 편집 기능 추가 요구
- Flutter 생태계에 적합한 라이브러리 부재 (초기 단계)
- 사용자/인플루언서 캠핑 컨텐츠 게시 및 도매처 상품 판매 서비스 제공

**Solution**

_커스텀 이미지 편집 라이브러리_

- **Dart 언어로 이미지 편집 라이브러리 직접 제작**
- Pinch-to-zoom, Cropping, Rotation, Flip 기능 구현
- 제스처(scale, pan, drag)에 맞게 aspect ratio, clip 처리
- Canvas API 활용한 저수준 이미지 처리

_크로스 플랫폼 개발_

- Flutter 기반 하이브리드 앱 개발 (Android/iOS)
- 단일 코드베이스로 개발 속도 향상

_백엔드 인프라_

- Firebase, GCP 활용
- Push messaging (FCM) 연동

**Achievement**

- 사용자 맞춤형 이미지 편집 기능 제공
- Flutter 생태계에 기여 가능한 라이브러리 개발 경험
- 라이브러리 제작하면서 Dart 언어에 대한 이해도 증가
- 캠핑 SNS 플랫폼 성공적 런칭
...
- **직급**: 서비스 연구원 (Platform Lead Engineer)
- **회사 소개**: AI 추천 솔루션 회사
...
_Kubernetes 네이티브 배포 자동화_

- **Argo Workflows + Helm** 기반 애플리케이션 배포 자동화 구축
...
- AI Fashion 추천 서비스 회사의 사업 확장 프로젝트
- Personal AI Stylist, Catalog Management 적용 필요
...
- POC 진행 시 mock 추천 시스템 제작 및 다양한 추천 알고리즘 테스트
...
- A/B 테스팅의 중요성
- POC 진행 시 mock 시스템 제작으로 완성도 없는 기술도 빠르게 검증 가능

---

#### 4.3. LOTTE Fashion SNS 추천 어드민 페이지

**Challenge**

- Fashion SNS APP에 사용되는 추천 서비스 관리도구 부재
- 각종 가중치를 조정하고 데이터 시각화 필요
- 서버에서 받아온 10만 건의 object 데이터를 브라우저에서 가공
- 낮은 사양 환경에서도 안정적 동작 필요

**Solution**

_데이터 처리 최적화_

- 10만 건의 object를 Vue.js로 효율적 가공
- 불필요한 반응성, 이벤트 최소화
- 데이터 가공 최적화 로직 구현

_데이터 시각화_

- **amchart**를 사용한 데이터 시각화
- 가중치 조정 UI 구현
- 대시보드를 통한 데이터 관리

_인프라_

- Kubernetes, ArgoCD를 통한 배포
- AWS 기반 인프라

**Achievement**

- 10만 건 object 데이터 실시간 처리 및 시각화
- 낮은 사양에서도 안정적 동작
- 추천 서비스 관리 효율 향상
...
- CJ The Market 상품 데이터를 대형 쇼핑몰로부터 수집 필요
- 쿠팡, 컬리, 이마트 등 각기 다른 구조의 쇼핑몰 크롤링
- 상품 데이터, 리뷰 데이터, 가격 데이터 등 다양한 데이터 수집
- ETL 작업 자동화 필요
...
- 각 쇼핑몰별 맞춤 크롤링 로직 구현
- 컬리의 경우 HTML에 노출된 API 키로 효율적 데이터 수집

_ETL 파이프라인_

- **Argo Workflows**를 사용한 ETL 작업 자동화
- Kubernetes 네이티브 워크플로우
- 수집된 데이터를 CJ The Market에 적재
...
#### 4.5. Fashion Magazine 데이터 수집 및 NLP

**Challenge**

- 국내/국외 패션 매거진 트렌드 분석 필요
- 키워드 추출을 통한 트렌드 파악
- 일자, 카테고리별 분류 및 검색 기능 제공

**Solution**

_데이터 수집 및 NLP_

- Fashion Magazine 사이트 데이터 수집
- **TF-IDF (Term Frequency-Inverse Document Frequency)** 알고리즘 구현
- 키워드 추출 및 자연어 처리
- 카테고리별 variation을 통한 최적 키워드 추출 알고리즘 구현

_검색 및 시각화_

- **Elastic Search** 연동으로 검색 기능 제공
- 일자, 카테고리별 분류
...
- Vue.js, Django 기반 웹 서비스 제작
- 파트너사 디자인 팀과 협업

**Achievement**

- 국내/국외 패션 매거진 트렌드 자동 분석 시스템 구축
- TF-IDF 알고리즘 직접 구현 및 최적화
- 다양한 variation을 통한 카테고리 맞춤 키워드 추출

**사용 기술**

- Python (BeautifulSoup, Selenium, TF-IDF, Pandas)
- Vue.js, Django
- Elastic Search
- Chart.js
...
- TF-IDF 알고리즘을 직접 구현하며 자연어 처리 이해도 향상
- 같은 기법도 variation을 통해 카테고리별 최적화 가능
- 데이터 분석과 시각화의 중요성

---

#### 4.6. 사내 AI 모델 관리 시스템 개선 및 유지보수

**Challenge**

- 40여 쇼핑몰 데이터 수집 및 정제 작업 관리
- 누끼, 스케일링, 검수 작업을 통한 유의미한 상품 데이터 관리
- AI 모델 데이터 증분 및 파이프라인 자동화
- Kubernetes 환경 서비스 관리
...
- **Dramatiq, RabbitMQ** 기반 비동기 처리
- Kubernetes 환경에서 서비스 관리
- Jenkins를 통한 CI/CD

_데이터베이스 관리_

- Django ORM을 통한 PostgreSQL 관리
- 데이터 처리 및 모델 학습용 데이터 제공

**Achievement**

- 40여 쇼핑몰 데이터 자동 수집 및 정제
- AI 모델 학습 파이프라인 자동화
- 컨테이너화된 서비스 관리 경험

**사용 기술**

- Python (Django, Dramatiq)
- RabbitMQ
...
- 사내 kubeadmin으로 구축된 Kubernetes 환경 관리 경험
- Django ORM을 통한 데이터베이스 관리
- AI 부서에 가공된 데이터를 제공하는 프로세스 이해
...
- 외주사 선정부터 프로젝트 진행까지 PM 역할
- WBS, 요구사항 정의서 작성
- 외주 개발자와의 협업 및 커뮤니케이션
- 개발 진행 상황 관리 및 산출물 검수
...
- 웹 에이전시의 UI/UX 디자인 및 개발 역량 파악
- 효율적인 협업 방법
- js 템플릿 엔진(EJS) 경험으로 스펙트럼 확장
- PM으로서 프로젝트 전체 관리 경험

---

#### 4.8. Virtual Try-on Backend & Frontend

**Challenge**

- AI 부서에서 제작한 가상 피팅룸 모델을 웹 서비스로 구현
- Intelligent Personalization Solution 적용
- 상/하의, 신발에 대한 3개 모델 별로 다른 서버 구성
- 국가 과제 프로젝트
